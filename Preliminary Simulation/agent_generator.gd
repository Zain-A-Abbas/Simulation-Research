...
func generate_agents(red_black_agents: RedBlackAgents):
	...
	if red_black_agents.scenario == red_black_agents.Scenarios.LONG_RANGE_CONSTRAINT:
		red_black_agents.count = red_black_agents.agent_count
		for agent in red_black_agents.agent_count:
			var starting_position: Vector2 = Vector2(red_black_agents.rng.randf() * red_black_agents.world_size.x, red_black_agents.rng.randf() * red_black_agents.world_size.y)
			red_black_agents.agent_positions.append(starting_position)
	...
	elif red_black_agents.scenario == red_black_agents.Scenarios.OPPOSING_AGENTS:
		red_black_agents.count = 2
	...
	elif red_black_agents.scenario == red_black_agents.Scenarios.OPPOSING_SMALL_GROUPS:
		red_black_agents.count = red_black_agents.agent_count
	...
	elif red_black_agents.scenario == red_black_agents.Scenarios.OPPOSING_LARGE_GROUPS:
		red_black_agents.count = red_black_agents.agent_count
		var agents_per_group: int = red_black_agents.count / 2
		var agents_per_row: int = sqrt(agents_per_group) / 2
		var rows: int = sqrt(agents_per_group) * 2
		...
		var agent_gap: Vector2 = Vector2(red_black_agents.radius * 1.25, red_black_agents.radius * 1.25)
		var group_positions: Array[Vector2] = [Vector2(100, 200), Vector2(100 + 2 * agents_per_row * agent_gap.x, 200 + red_black_agents.radius / 2.0)]
		var group_velocities: Array[Vector2] = [Vector2(red_black_agents.max_velocity, 0), Vector2(-red_black_agents.max_velocity, 0)]
		for z in 2:
			for row in rows:
				for row_position in agents_per_row:
